...
describe('ProtectedRoute', () => {
  ...
  describe('약관 동의 가드 (needsTermsAgreement)', () => {
    it('needsTermsAgreement=true 이고 pathname이 / 이면 /terms-agreement 로 redirect 한다', () => {
      useAuthStoreMock.mockImplementation((sel: any) =>
        sel({
          user: {
            id: '1',
      ...
      // Routes를 사용해 redirect 대상을 sentinel 으로 분리 — Navigate 후 MemoryRouter가
      // /terms-agreement 로 이동해도 protected-child가 없고 terms-sentinel이 렌더됨을 확인
      renderWithTheme(
    ...
    it('needsTermsAgreement=true 이고 pathname이 /terms-agreement 이면 children을 렌더한다', () => {
      useAuthStoreMock.mockImplementation((sel: any) =>
        sel({
          user: {
            id: '1',
      ...
      renderWithTheme(
        <MemoryRouter initialEntries={['/terms-agreement']}>
          <ProtectedRoute>
            <div data-testid="terms-child">Terms Page</div>
          </ProtectedRoute>
        </MemoryRouter>,
      );

      expect(screen.getByTestId('terms-child')).toBeInTheDocument();
    });

    it('needsTermsAgreement=true 이고 pathname이 /terms 이면 children을 렌더한다', () => {
      useAuthStoreMock.mockImplementation((sel: any) =>
        sel({
          user: {
            id: '1',
    ...
    it('needsTermsAgreement=true 이고 pathname이 /privacy 이면 children을 렌더한다', () => {
      useAuthStoreMock.mockImplementation((sel: any) =>
        sel({
          user: {
            id: '1',
      ...
      renderWithTheme(
        <MemoryRouter initialEntries={['/privacy']}>
          <ProtectedRoute>
            <div data-testid="privacy-child">Privacy Page</div>
          </ProtectedRoute>
        </MemoryRouter>,
      );

      expect(screen.getByTestId('privacy-child')).toBeInTheDocument();
    });

    it('needsTermsAgreement=false 이면 어떤 경로든 children을 렌더한다', () => {
      useAuthStoreMock.mockImplementation((sel: any) =>
        sel({
          user: {
            id: '1',
    ...
    it('needsTermsAgreement=undefined (기존 사용자) 이면 children을 렌더한다', () => {
      useAuthStoreMock.mockImplementation((sel: any) =>
        sel({
          user: { id: '1', username: 'testuser', email: 'test@example.com' },
          isLoggedIn: true,
    ...
    it('isLoggedIn=false 이면 needsTermsAgreement 값과 무관하게 /login 으로 redirect 한다', () => {
      useAuthStoreMock.mockImplementation((sel: any) =>
        sel({
          user: {
            id: '1',
